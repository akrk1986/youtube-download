...
"""qBittorrent post-download hook (Slack): flag DoVi profile 5, then post to Slack.

Wire this as the qBittorrent "Run on torrent finished" command:
    python /path/to/Utils/main-qb-postdownload-slack.py --name "%N" --path "%F"

It runs the DoVi profile-5 detection on the downloaded content, then invokes
main-qb-notify-slack.py with the resulting good/bad status. The detection +
notifier-invocation logic is shared with the Gmail driver via
...
_NOTIFIER_SCRIPT = 'main-qb-notify-slack.py'


def main(argv: list[str] | None = None) -> None:
    """Run the hook and exit with the Slack notifier's return code.
