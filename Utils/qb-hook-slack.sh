#!/bin/sh
# qBittorrent "Run external program on torrent completion" wrapper (Slack).
#
...
DRIVER="$SCRIPT_DIR/main-qb-postdownload-slack.py"

if [ ! -x "$VENV_PY" ]; then
    echo "qb-hook-slack: venv python not found at $VENV_PY" >&2
    exit 1
fi
