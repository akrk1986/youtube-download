...
This guide explains how to configure mp3tag to properly display and edit metadata tags in MP3 and M4A files generated by this yt-dlp-based downloader.

## Overview

This project generates MP3 and M4A audio files with specific metadata tags that may require custom configuration in mp3tag to display correctly. The tool embeds metadata including:

- Standard tags (title, artist, album, year, genre)
- Embedded album artwork (thumbnail from YouTube)
- Custom tags for tracking original filenames (TENC for MP3, ©lyr for M4A)
...
5. **Keep both formats separate** when batch editing to avoid format-specific issues
...
| Feature | MP3 (ID3v2) | M4A (MP4/iTunes) |
|---------|-------------|------------------|
| Original filename storage | TENC (Encoded by) | ©lyr (Lyrics) |
| Date format | YYYY | YYYY (auto-fixed from YYYYMMDD) |
| Encoding | UTF-8 (ID3v2.4) or UTF-16 (ID3v2.3) | Always UTF-8 |
| Greek text support | Requires ID3v2.4 UTF-8 | Native UTF-8 support |
| Album art tag | APIC frame | covr atom |
| Track number format | String ('1', '2', etc.) | Tuple (track, total_tracks) |

## Notes

- This project supports both MP3 and M4A audio formats
- MP3 uses ID3v2 tags; M4A uses MP4/iTunes-style atoms
- Chapter-split files are automatically organized into subdirectories
- M4A format provides better Unicode support out of the box
