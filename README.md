...
- Extract audio as MP3 and/or M4A with embedded metadata and thumbnails
...
```
usage: main-yt-dlp.py [-h] [--with-audio] [--audio-format {mp3,m4a,both}]
...
options:
  ...
  --audio-format {mp3,m4a,both}
                        Audio format for extraction: mp3, m4a, or both (default: mp3)
...
You can choose the output audio format:

- `--audio-format mp3` (default) - Extract as MP3 with ID3v2 tags
- `--audio-format m4a` - Extract as M4A with MP4/iTunes atoms
- `--audio-format both` - Extract both MP3 and M4A formats

**Example:**
```bash
# Extract both formats
python main-yt-dlp.py --with-audio --audio-format both "https://youtube.com/watch?v=VIDEO_ID"

# M4A only
python main-yt-dlp.py --only-audio --audio-format m4a --split-chapters "https://youtube.com/playlist?list=PLxxxxxxxx"
```
...
### Download playlist with both audio formats
```bash
python main-yt-dlp.py --only-audio --audio-format both "https://youtube.com/playlist?list=PLxxxxxxxx"
```

## Output Structure

- `yt-videos/` - Downloaded MP4 video files
- `yt-audio/` - Downloaded MP3/M4A audio files
...
- **[Configuring mp3tag to Display yt-dlp Generated Audio Files](Docs/Configuring%20mp3tag%20to%20display%20yt-dlp%20generated%20audio%20files.md)** - Guide for viewing and editing metadata in mp3tag for both MP3 and M4A files
