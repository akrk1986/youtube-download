...
- **Clipboard watcher** (`services/clipboard_watcher.py`): the Start/Stop-watching buttons show **only for `YT-DLP-prompt` presets** (the URL-prompting group, via `is_prompt_preset` / `FormView.current_is_prompt`); the quick `YT-DLP-presets` **pre-fill their hardcoded playlist URL** and the linters take no URL, so both hide the buttons. `_is_youtube_url` matches video / playlist / `youtu.be` / `shorts` / `music.youtube.com` links. The clipboard is read from the OS (not the browser) via pyperclip — most reliable run natively (esp. Windows).
