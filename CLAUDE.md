...
- **Listens on loopback**: `DEFAULT_HOST` (`webapp/config.py`) and `webapp/config.json` are both `127.0.0.1`. The form builds argv for `main-yt-dlp.py`, so LAN exposure is opt-in — `--host 0.0.0.0`, `WEBAPP_HOST`, or the config key (precedence: CLI > env > config.json > default). Binding `0.0.0.0` also made NiceGUI's startup banner print one URL per network adapter (Hyper-V/WSL vEthernet, APIPA link-local), almost none reachable; the `--native` window loads `localhost` either way.
- **Cookie default is `none`**: `webapp/config.json` sets `"cookies": "none"` explicitly, so no run carries browser cookies unless the **Cookies** dropdown asks for them. A *blank* value falls back to `_platform_default_cookies()` — `firefox` on native Windows, `none` on WSL/Linux/macOS (the Windows Firefox profile is unreachable there).
- **Exit and Abort take platform-specific routes**: in `--native` mode Exit destroys the webview window and lets NiceGUI's watcher hard-exit (`app.shutdown()` drains a websocket whose peer is already gone and raises `LocalProtocolError` out of `ui.run()`); Abort kills the whole process tree — `killpg` on POSIX (the driver gets `start_new_session`), `taskkill /T /F` on Windows — so the driver's yt-dlp child is not orphaned.
- **Module split**: UI-free, unit-tested core (`config.py`, `presets.py`, `runner.py`, `validate.py`, `ansi.py`) imports no nicegui; the nicegui shell (`form.py`, `app.py`) does. Tests: `Tests/test_webapp.py` (no NiceGUI runtime).
- **Clipboard watcher** (`services/clipboard_watcher.py`): the Start/Stop-watching buttons show **only for `YT-DLP-prompt` presets** (the URL-prompting group, via `is_prompt_preset` / `FormView.current_is_prompt`); the quick `YT-DLP-presets` **pre-fill their hardcoded playlist URL** and the linters take no URL, so both hide the buttons. `_is_media_url` matches YouTube (video / playlist / `youtu.be` / `shorts` / `music.youtube.com`) **and Facebook video** links (`watch?v=`, `<page>/videos/`, `video.php?v=`, `reel/`, `share/v|r/`, `fb.watch`); `_extract_media_url` first unwraps the `google.com/url?q=…` redirect that Gmail puts around copied links. A media URL already on the clipboard is delivered immediately on 'Start watching' (copy-first flow). The clipboard is read from the OS (not the browser) via pyperclip; `Tests-Standalone/main-clipboard-probe.py` diagnoses interception misses. **Disabled under WSL** (`is_wsl()` in `config.py` gates both button visibility and the poll timer — the Windows clipboard bridge is unreliable from the running server); active on Windows and native Linux, though **native Linux is untested** (see `webapp/README.md` "Known issues").
- **Output log renders colour**: the linter subprocess is launched with `FORCE_COLOR=1` + `COLUMNS=120` and every Run-Linters preset passes `--batch` (skips the freshness upgrade-script prompt, which the browser can't answer); `webapp/ansi.py` converts the ANSI/emoji output to HTML for the monospace log. **The ANSI palette is theme-dependent** — `DARK_PALETTE` / `LIGHT_PALETTE` chosen via `palette_for(dark=…)`, because SGR 30 (black) and 97 (bright white) are invisible on whichever background matches them; every entry clears 4.5:1 on its background, and `Tests/test_webapp.py` asserts that. Log lines are **queued and flushed every 100 ms** as one element (`_AnsiLog` in `app.py`), not rendered per line.
...
| `freshness` | Outdated packages with release age + New/Stable badge; generates the platform upgrade script (informational, opt-in) |
| `pyupgrade` | Syntax modernisation (modifies files in place) |
...
- `freshness` always exits 0 (opt-in via `--tool freshness`; needs network). Venv-scoped, so output is identical from any sibling; pre-releases and intentional caps (e.g. `skylos <4.12`) are excluded, mirroring what `pip-compile --upgrade` can move. Renders a `rich` table (Package / Current / Latest / Age (days) / Action) where **Age** is days since the latest release was published on PyPI with a red **`New`** (`< 8` days) / green **`Stable`** (`≥ 8` days) badge, and **Action** flags sdist-only releases as `⚠ build from source`. It then writes `pip-upgrade-stable.sh` **or** `pip-upgrade-stable.ps1` (gitignored) containing only the Stable packages — only the script for the OS it is running on, since the per-OS venvs differ and a cross-OS script installs packages against constraints that never applied there; the generated header records platform and Python version. It then interactively offers to display/run it (non-interactive callers skip the prompts). Implemented by `run_freshness()` in the shared `common_linters/freshness.py`
