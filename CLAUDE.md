...
- **Output log renders colour**: the linter subprocess is launched with `FORCE_COLOR=1` + `COLUMNS=120` and every Run-Linters preset passes `--batch` (skips the freshness upgrade-script prompt, which the browser can't answer); `webapp/ansi.py` converts the ANSI/emoji output to HTML for the monospace log.
