...
Executes real download scenarios with actual URLs from e2e_config.py.
...
def _check_prerequisites() -> bool:
    ...
    if isinstance(ytdlp_path, Path):
        ...
        result = subprocess.run(  # nosec B603 B607
            ['which', 'yt-dlp'], capture_output=True, text=True, encoding='utf-8', errors='replace'
        )
    ...
    if isinstance(ffmpeg_path, Path):
        ...
        # Check if in PATH
        result = subprocess.run(  # nosec B603 B607
            ['which', 'ffmpeg'], capture_output=True, text=True, encoding='utf-8', errors='replace'
        )
        if result.returncode != 0:
...
Configure test URLs in: Tests/e2e_config.py
        """,
    )
    parser.add_argument(
        '--resume',
